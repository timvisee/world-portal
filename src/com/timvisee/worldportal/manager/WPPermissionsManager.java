...
public class WPPermissionsManager {
	...
	public PermissionsSystemType setUp() {
		...
		// Permissions
		// Check if Vault is allowed to be used
		if(isPermissionsSystemAllowed("Permissions")) {
			...
			try {
		        ...
		        if (testPerms != null) {
		            ...
		            this.log.info("Hooked into Permissions!");
		            return PermissionsSystemType.PERMISSIONS;
		        }
			} catch(Exception ex) {
				// An error occurred, show a warning message
				this.log.error("Error while hooking into Permissions!");
			}
		} else {
			// Show a warning message
			this.log.info("Not checking for Permissions, disabled in config file!");
		}
	...
	public boolean hasPermission(Player p, String permsNode, boolean def) {
		...
		case PERMISSIONS: // Permissions by nijiko
