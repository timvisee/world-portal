...
import com.timvisee.worldportal.command.FoundCommandResult;
...
public class HelpProvider {
    ...
    public static void showHelp(CommandSender sender, CommandParts reference, CommandParts helpQuery, boolean showCommand, boolean showDescription, boolean showArguments, boolean showPermissions, boolean showAlternatives, boolean showCommands) {
        ...
        FoundCommandResult result = WorldPortal.instance.getCommandHandler().getCommandManager().findCommand(new CommandParts(helpQuery.getList()));
        CommandParts commandReferenceOther = new CommandParts(reference.get(0), helpQuery.getList());
        FoundCommandResult resultOther = WorldPortal.instance.getCommandHandler().getCommandManager().findCommand(commandReferenceOther);
        if(resultOther != null) {
            if(result == null)
                result = resultOther;
        ...
        sender.sendMessage(ChatColor.GOLD + "==========[ " + WorldPortal.getPluginName().toUpperCase() + " HELP ]==========");
