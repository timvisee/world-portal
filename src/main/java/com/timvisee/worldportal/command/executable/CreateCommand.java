...
public class CreateCommand extends ExecutableCommand {
    ...
    public boolean executeCommand(CommandSender sender, CommandParts commandReference, CommandParts commandArguments) {
        ...
        if(!(sender instanceof Player)) {
            sender.sendMessage(ChatColor.DARK_RED + "You can only create portals in-game.");
            return true;
        }
        ...
        if(enable)
            WorldPortal.instance.createPortal.toggleWPUsers((Player) sender);
        else if(WorldPortal.instance.createPortal.isPlayerInCreationMode((Player) sender))
            WorldPortal.instance.createPortal.toggleWPUsers((Player) sender);
        return true;
    }
}
